...
function ItemDetails() {
  ...
  async function getItem() {
    const item = await fetch(
      `https://smiling-joy-b1e870043b.strapiapp.com/api/items/${itemId}?populate=deep`,
      { method: 'GET' }
    );
  ...
  async function getItems() {
    const items = await fetch(
      "https://smiling-joy-b1e870043b.strapiapp.com/api/items?populate=deep",
      { method: "GET" }
    );
            ...
            <Box sx={{ minWidth: 120 }} mr="20px" p="2px 5px">
              <FormControl required fullWidth>
                ...
                <Select
                  ...
                  onChange={(e) => { 
                    const setImg = item?.attributes?.thumbnails.data.find((item) => item?.attributes?.alternativeText.includes(e.target.value));
                    setMainImgUrl(setImg?.attributes?.url);
